...
from stories.models import Story, StoryItem, StoryText, StoryGraph
...
def test_make_storygraph(user,other_user, experiment_with_result, model_with_version, public_model, public_protocol):
    ...
    #make
    story_graph = recipes.story_graph.make(author=user, story=stories[0], cachedprotocolversion=experiment.protocol_version,
    ...
    # change
    modelgroup = recipes.modelgroup.make(author=user, models=[model_with_version, public_model], title="test model group")
    ...
    assert sorted([m.model for m in story_graph.cachedmodelversions.all()], key=str) == sorted([model_with_version,
                                                                                                public_model], key=str)
