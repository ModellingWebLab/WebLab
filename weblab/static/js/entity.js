  ...
  */
  ...
  function triggerExperimentSubmission(plannedExperiments, submitNow)
  {
    if (plannedExperiments.length > 0 && submitNow)
    {
      ...
      $.post($("#entityversion").data('new-expt-href'), exptSpec, function(data) {
...
$(document).ready(function(){
    ...
    $('.deselectModelFromGroup').click(function(){
        ...
        $(`#${my_id_prefix}availableModels`).append($(`#${my_id_prefix}id_models`).find(":selected"));
        set_visibility();
    });
    $('.slectModelForGroup').click(function(){
        my_id_prefix = $(this).attr('id').replace('slectModelForGroup', '');
        $(`#${my_id_prefix}id_models`).append(selected = $(`#${my_id_prefix}availableModels`).find(":selected"));
        set_visibility();
    });
